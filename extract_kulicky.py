...
def extract_all_kulicky():
    ...
    try:
                ...
                try:
                    ...
                    if text:
                        ...
                        for line in lines:
                            ...
                            if 'KULIČKY' in line:
                                in_kulicky_section = True
                                continue
                            
                            # Pokud jsme v sekci kuliček, sbíráme je
                            if in_kulicky_section and current_lesson and current_lesson in lessons_kulicky:
                                ...
                                if (line and 
                                    ...
                                    if clean_line and clean_line not in lessons_kulicky[current_lesson]:
                                        lessons_kulicky[current_lesson].append(clean_line)
                            
                            # Také hledáme kuličky, které začínají tečkami nebo čárkami
                            elif current_lesson and current_lesson in lessons_kulicky:
        ...
        # Vypíšeme výsledky
        print_results(lessons_kulicky)
        
    except Exception as e:
        print(f"Chyba při otevírání PDF: {e}")

def print_results(lessons_kulicky):
    ...
    for lesson_num, kulicky in lessons_kulicky.items():
        print(f"\n📚 LEKCE {lesson_num}: {lesson_names[lesson_num]}")
        print(f"Počet kuliček: {len(kulicky)}")
        print("-" * 60)
        
        for i, kulicka in enumerate(kulicky[:20], 1):  # Zobrazíme prvních 20
            print(f"{i:2d}. {kulicka}")
        
        if len(kulicky) > 20:
            print(f"... a dalších {len(kulicky) - 20} kuliček")
